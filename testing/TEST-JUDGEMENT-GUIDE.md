...
### KJ-2026-0924-01 staging に残っていた過去のテストデータが、新しい制約を追加するリリースを止めた

- **該当シナリオ**: `supabase/release/20260919_feature-20260918-dev_release.sql` のセクション5
  （`com_t_user_license.excl_user_license_active_overlap` 排他制約の追加）を
  `supabase/release/run.mjs` で staging に適用した際に発覚
- **事象**: セクション5冒頭の事前チェックで「期間が重なる有効ライセンスが5組」と検出され、
  リリースが停止した。該当はすべて 2026-09-17 の staging 検証（tag: `stg0917`）で作成された
  QA生徒3名（`stg0917-24h-student-t1` / `stg0917-mystudents-student-a` / `stg0917-mreport-student`）の
  ライセンスで、同一ユーザーに別契約・同一期間の有効ライセンスが2〜3件ずつ存在していた。
- **原因**: seedスクリプトがアプリ側の重複チェック（`findOverlappingLicense`）を通さず、
  service_role で `com_t_user_license` に直接INSERTしていたため、業務ルール上ありえない状態の
  データが staging に残っていた。制約を後から追加するリリースでは、こうした過去のテストデータが
  既存データ違反として顕在化する。
- **対処**: 各ユーザーで最初に登録された1件を残し、残り4件を `status=0`（無効）に更新した
  （自動テスト用アカウントのため影響なしと開発者確認済み。排他制約は `status=1` のみが対象）。
  その後 `run.mjs --sections=pending` でセクション5から再開し、全セクション適用済みを確認した。
- **判断基準への反映**:
  - **seedスクリプトで業務テーブルへ直接INSERTする場合も、業務ルール（同一ユーザーの有効ライセンスは
    期間が重ならない等）に反するデータは作らないこと。** 今は制約が無くても、後から制約を追加する
    リリースで staging の適用を止める原因になる。
  - **既存データを検証してから制約を追加するセクション（DOブロックの事前チェック等）は、
    staging 適用で止まる前提で、本番適用前に同じ条件の読み取りクエリで本番データを確認しておくこと。**

### KJ-2026-0924-02 生徒モニタリングの期間判定で、翌月1日開始のライセンスが前月の一覧にも出る（JST保存×UTC比較）

- **該当シナリオ**: `testing/features/branches/feature-20260918-dev/monitor-target-period.feature`
  （staging, tag=stg0924。固定フィクスチャ `qa-student-05` = 次期2026-12-01開始のみ）
- **事象**: 対象期間 2026-11-01〜11-30 で `get_monitor_user_list` を呼ぶと、12/1開始の生徒05が
  `license_state='future'` で一覧に含まれた。単語ドリル等の実績RPCも同じ
  `private.get_monitor_target_users` を使うため、翌月1日開始の生徒の実績が前月末日に存在すれば同様に表示される。
- **原因**: ライセンスは管理画面の登録形式どおりJSTの日境界で保存される（12/1 00:00 JST =
  `2026-11-30 15:00+00`）。一方 `get_monitor_target_users` は `l.start_date < (_end_date + 1)::timestamptz`
  のように DATE を DBのセッションタイムゾーン（UTC）で timestamptz に変換して比較しているため、
  JSTで翌月1日開始のライセンスが、UTC上は前月末日の15:00から重なっていると判定される。
  終了側（JST末日 23:59:59 = UTC 14:59:59）は翌月の期間と重ならないため問題が出ない（開始側のみの片側ズレ）。
- **対処**: テストはNGのまま記録し、開発者へ報告した（期待値は「含まれない」が正）。修正する場合は
  `(_end_date + 1)::timestamp AT TIME ZONE 'Asia/Tokyo'` のようにJSTの日境界で比較する形が候補。
- **判断基準への反映**:
  - **DATE型の期間パラメータとtimestamptz列を比較するRPCは、「JSTの月初1日開始／月末日終了」の
    データで境界の前後の期間を必ず検証する。** 片側だけズレるため、期間の中間月だけを見ていると気づけない。
  - 固定フィクスチャのタームはJST月初開始なので、「次期開始の直前月」「前期最終月の翌月」を
    期間に指定するだけでこの種の境界ズレを検出できる。

### KJ-2026-0924-03 過去テストデータの一括削除は、命名規則による機械的な対象判定と「またぎ検知」の安全装置で行う

- **該当シナリオ**: `testing/features/fixtures/purge-disposable-qa-data.sql`（staging, 2026-09-24）
- **事象**: staging に過去のブランチ検証（tag=`auto0910` / `stg0917`）の使い捨てデータ
  （顧客8件・ユーザー45名と配下の契約・セッション等）が残っており、KJ-2026-0924-01 のように
  後続リリースの制約追加を止める原因にもなっていた。シナリオごとの`cleanup.ts`は一部のシナリオにしか無く、
  FKにCASCADEが無いテーブル（KJ-2026-0914-02 / 0915-01）も多いため、個別に消すのは現実的でなかった。
- **原因**: 固定アカウント（残すもの）と使い捨てデータ（消すもの）を区別する規則が文書化されておらず、
  一括で安全に削除する手段が無かった。
- **対処**: `pg_constraint`から`com_m_user`/`com_m_client`/`com_m_contract`を根とするFK依存ツリーを洗い出し、
  子から順に削除する1トランザクションのSQLにまとめた。対象は命名規則（メールが`qa-`始まりでない
  `@gabby-qa-test.example`、顧客名が`【QAテスト】`始まり）だけで判定し、「対象顧客に対象外ユーザーが所属」
  「対象ユーザーと対象外ユーザーをまたぐセッション・担当関係」があれば RAISE で全体を中止する。
  まず末尾を`RAISE EXCEPTION`に差し替えたdry-runでFK順序と安全装置を確認してから本実行した。
  ユーザーから削除の明示的な許可を得た上で、Management API経由のSQL実行はブロックされずに完了した。
- **判断基準への反映**:
  - **固定/使い捨ての区別は命名で機械的に判定できるようにしておく（`FIXTURES.md`）。** 「どれを残すか」を
    毎回人手で判断しないで済み、一括削除スクリプトも安全装置付きで再利用できる。
  - **Management API（`/database/query`）での破壊的SQLのdry-runは、末尾のCOMMITをROLLBACKに変えるのでは
    なく、例外（RAISE EXCEPTION）で終わらせる。** APIは最後の文の結果しか返さず、接続が
