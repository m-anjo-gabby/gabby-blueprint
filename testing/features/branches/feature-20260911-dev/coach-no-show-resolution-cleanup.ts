/**
 * coach-no-show-resolution-seed.ts で投入したテストデータを削除する。
 * 本シナリオはユーザーから「テスト完了後に削除してよい」と明示的な指示があったため、
 * verify.ts実行後に本スクリプトを実行してテストデータを後始末する
 * (CLAUDE.md 6章の原則: 明示指示が無い限り残す、の例外運用)。
 *
 * 削除は一括DELETEではなく、本タグで作成した顧客(client_id)・コーチ2名・生徒2名の
 * IDに厳密に絞ったスコープで、FK依存順に1テーブルずつ行う(冪等: 対象が無ければ何もしない)。
 * 共有フィクスチャであるqa-admin@gabby-qa-test.exampleアカウントは削除しない。
 * 削除順序はsession-lifecycle-refactor-cleanup.tsと同じFK依存関係に基づく
 * （KJ-2026-0914-02: com_t_session.ticket_idはCASCADE無し、KJ-2026-0915-01も参照）。
 *
 * 使い方:
 *   pnpm exec tsx testing/features/branches/feature-20260911-dev/coach-no-show-resolution-cleanup.ts --env=dev --tag=noshow01
 */
...
console.log(`\n=== コーチ無断欠席解決 テストデータ削除: env=${env} tag=${TAG} ===`);

const clientName = `【QAテスト】コーチ無断欠席解決検証（${TAG}）`;
...
const emails = [
  `${TAG}-noshow-coach@gabby-qa-test.example`,
  `${TAG}-noshow-other-coach@gabby-qa-test.example`,
  `${TAG}-noshow-student-ns@gabby-qa-test.example`,
  `${TAG}-noshow-student-nm@gabby-qa-test.example`,
];
...
//    無いため、ticket/licenseを消す前に明示的に削除する必要がある）
{
  ...
  if (ticketIds.length > 0) {
    ...
    if (error) throw error;
    console.log(`2. com_t_session削除(連鎖でcall_logも削除): ${count ?? 0}件`);
  } else {
    console.log("2. com_t_session削除: 対象チケットなし(0件)");
  }
}

// 3. ライセンス（CASCADE: ticket→schedule まで連鎖。sessionは既に2で削除済み）
{
