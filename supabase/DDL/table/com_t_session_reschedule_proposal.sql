...
-- コーチ側のPending取得(getPendingIncomingRescheduleProposalGroupsForCoachCore)はcoach_id+statusで
-- 絞り込むため、com_t_matching_request/com_t_session_booking_requestと同様にこちらにも必要 (2026-09-12 追加)
CREATE INDEX idx_session_reschedule_proposal_coach_status ON public.com_t_session_reschedule_proposal (coach_id, status);
-- コーチ側「申請一覧」画面のHistoryタブ(getRescheduleProposalHistoryPageForCoachCore)は
-- coach_id絞り込み + insert_date降順のカーソルページングのため必要 (2026-09-14 追加)
CREATE INDEX idx_session_reschedule_proposal_coach_insert_date ON public.com_t_session_reschedule_proposal (coach_id, insert_date DESC);
