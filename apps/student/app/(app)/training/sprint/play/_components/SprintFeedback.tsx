...
export const SprintFeedback: React.FC<SprintFeedbackProps> = ({ 
  feedback, 
  analysis, 
  onClose 
}) => {
  ...
  return (
    <AnimatePresence>
      <motion.div 
        className="absolute inset-0 z-[110] flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-md"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        onClick={handleClose}
      >
        <motion.div 
          className="relative bg-white w-full max-w-sm rounded-[36px] p-6 shadow-2xl border border-white/50 flex flex-col items-center gap-6"
        ...
        >
          <div className="w-full flex justify-between items-center px-1">
            ...
            <button onClick={handleClose} className="p-2 -mr-2 text-slate-300 hover:text-slate-500 transition-colors">
              <X size={20} strokeWidth={2.5} />
            </button>
          </div>
          
          <div className="flex items-center gap-6 w-full bg-slate-50 p-5 rounded-[28px] border border-slate-100/50">
            ...
            <div className="flex-1 flex flex-col gap-1.5">
              <div className="inline-flex px-2.5 py-0.5 rounded-full w-fit bg-white border text-[9px] font-black uppercase tracking-wider" style={{ color: feedback.fill, borderColor: `${feedback.fill}20` }}>
                {feedback.tagText}
              </div>
          ...
          <div className="flex flex-wrap justify-center gap-x-3 gap-y-4 px-2 max-h-[160px] overflow-y-auto w-full py-1">
            {analysis.matches.map((m, idx) => {
              ...
              return (
                ...
                >
                  <span className={cn("text-lg font-black transition-all duration-300", text, deco)}>
                    {m.word}
                  </span>
                </div>
              );
          ...
          {analysis.issues && analysis.issues.length > 0 && (
            <div className="w-full bg-indigo-50/50 p-4 rounded-[24px] border border-indigo-100/50">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-[10px] font-black uppercase text-indigo-500 tracking-wider">Analysis Result</p>
              </div>
