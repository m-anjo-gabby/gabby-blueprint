...
export function StudentSessionResult({ session, homework, checklist: initialChecklist }: Props) {
  ...
  const sprintMetric = getTrainingMetricConfig('sprint');
    ...
    <>
      ...
      <div className="space-y-4">
        ...
        <section className="rounded-card border border-line bg-surface p-4 space-y-3">
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-10 h-10 rounded-control bg-brand-soft overflow-hidden flex items-center justify-center text-brand-500 shrink-0">
                {coachIconUrl ? (
              ...
              <p className="text-sm sm:text-base font-bold text-ink">{session.counterpart_name} コーチ</p>
            </div>
          </div>
          ...
          <div className="pt-1 border-t border-line">
            ...
            >
              ...
              <motion.span animate={{ rotate: showCallLog ? 180 : 0 }} className="text-ink-subtle shrink-0">
                <ChevronDown size={16} />
              </motion.span>
            </button>
                ...
                >
                  <div className="pt-2.5">
                    {session.call_log.length === 0 ? (
                      <p className="text-xs text-ink-subtle italic">通話記録はありません。</p>
                    ) : (
                      <ul className="space-y-2">
                        {session.call_log.map((entry) => (
                          <li key={entry.call_log_id} className="flex items-start gap-2 text-xs">
                            <span className={`mt-0.5 shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${entry.role === 'coach' ? 'bg-canvas text-ink-muted' : 'bg-brand-soft text-brand'}`}>
        ...
        <section className="rounded-card border border-line bg-surface p-4 space-y-4">
          <SectionHeading icon={BookOpen} iconClassName="bg-brand-soft text-brand" title="宿題" />

          {!homework ? (
            <p className="text-xs text-ink-subtle italic">まだ宿題はありません。</p>
          ) : (
            <>
              <div className="rounded-xl border border-line bg-canvas px-3.5 py-3 space-y-2">
                <p className="text-[11px] font-bold text-ink-subtle">{formatDateTimeByZone(homework.insert_date, timezone, false)}</p>
                <p className="text-sm text-ink-soft whitespace-pre-wrap wrap-break-word">{linkifyText(homework.homework_text)}</p>
              ...
              {checklist.length > 0 && (
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-[11px] font-bold text-brand tabular-nums shrink-0">
                          ...
                          >
                            <span
                              className={`shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${
                                item.is_done ? 'bg-emerald-500 border-emerald-500' : 'border-ink-subtle bg-surface'
                              }`}
                            >
                              <AnimatePresence>
                                {item.is_done && (
                            ...
                            <span className={`text-sm flex-1 ${item.is_done ? 'text-ink-subtle line-through' : 'text-ink-soft font-medium'}`}>
                              {item.item_text}
                            </span>
                          </button>
                        </motion.li>
              ...
              <div className="space-y-2.5 pt-1 border-t border-line">
                <div className="flex items-center gap-1.5 text-ink-muted pt-2">
                  <MessageSquareText size={13} />
                  <p className="text-xs font-bold">フォローアップコメント</p>
                </div>

                {homework.comments.length === 0 ? (
                  <p className="text-xs text-ink-subtle italic">フォローアップコメントはまだありません。</p>
                ) : (
                  <div className="space-y-3">
                    {homework.comments.map((comment) => (
                      <div key={comment.comment_id} className="rounded-xl border border-line bg-canvas px-3.5 py-3 space-y-2">
                        <p className="text-[11px] font-bold text-ink-subtle">{formatDateTimeByZone(comment.insert_date, timezone, false)}</p>
                        {comment.comment_text && (
                          <p className="text-sm text-ink-soft whitespace-pre-wrap wrap-break-word">{linkifyText(comment.comment_text)}</p>
                        )}
        ...
        <section className="rounded-card border border-line bg-surface p-4 space-y-4">
          <SectionHeading icon={sprintMetric.icon} iconClassName={sprintMetric.theme.iconTile} title="トレーニング" />

          <div className="space-y-2">
            <p className="text-xs font-bold text-ink-muted">Live Sprint</p>
            {session.sprint_log.length === 0 ? (
              <p className="text-xs text-ink-subtle italic">このセッションでは実施されませんでした。</p>
            ) : (
              <ul className="space-y-2">
                {session.sprint_log.map((entry) => (
                  <li key={entry.lesson_sprint_id} className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl border border-line bg-canvas">
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-ink-soft truncate">{entry.content_name}</p>
                      <p className="text-[11px] text-ink-subtle">{formatDateTimeByZone(entry.insert_date, timezone, false)}</p>
                    </div>
                    <span className="shrink-0 text-[11px] font-bold text-brand bg-brand-soft border border-brand-100 rounded-full px-2.5 py-1">
          ...
          {/*
            ...
            <div className="space-y-2 pt-1 border-t border-line border-dashed">
              ...
              <p className="text-xs text-ink-subtle italic">近日公開予定です。</p>
            </div>
          */}
        </section>
        ...
        {/* チャット履歴 */}
        <section className="rounded-card border border-line bg-surface p-4 space-y-3">
          <SectionHeading icon={MessageCircle} iconClassName="bg-brand-soft text-brand" title="チャット履歴" />
          {session.chat_log.length === 0 ? (
            <p className="text-xs text-ink-subtle italic">通話中のチャットメッセージはありません。</p>
          ) : (
            <ul className="space-y-2">
              {session.chat_log.map((entry) => (
                <li key={entry.chat_id} className={`text-xs ${entry.sender_role === 'student' ? 'text-right' : 'text-left'}`}>
                  <p className="font-bold text-ink-subtle text-[11px]">{entry.sender_role === 'coach' ? 'コーチ' : '自分'}</p>
                  <p
                    className={`inline-block mt-0.5 px-2.5 py-1.5 rounded-lg whitespace-pre-wrap wrap-break-word ${
                      entry.sender_role === 'student' ? 'bg-brand-soft text-brand-strong' : 'bg-canvas text-ink-soft'
                    }`}
                  >
                    {entry.message}
                  </p>
...
function HomeworkAttachmentView({ attachment }: { attachment: SessionHomeworkAttachment }) {
  ...
  if (isLoading) {
    return <Loader2 size={16} className="animate-spin text-ink-subtle" />;
  }

  if (!url) {
    return <p className="text-xs text-ink-subtle">添付ファイルの読み込みに失敗しました</p>;
  }
