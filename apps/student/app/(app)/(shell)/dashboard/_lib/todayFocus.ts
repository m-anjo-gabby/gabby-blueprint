 ...
 * 優先順位: 開始が近いライブセッション > コーチからの未完了の課題 > 学習開始の案内
 * 途中の教材の再開（ブックマーク）はここでは扱わず、補助カード「続きから」に表示する。
 */
export function resolveTodayFocus({ nextSession, assignments, nowMs }: ResolveTodayFocusInput): TodayFocus {
  if (nextSession) {
