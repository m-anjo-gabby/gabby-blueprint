      ...
      <PopoverContent className="w-64" align="center">
        <div className="flex items-center justify-between mb-3">
          <button
            ...
            className="p-1 rounded-md text-ink-muted hover:bg-slate-100"
            aria-label="前年"
          >
            <ChevronLeft size={16} />
          </button>
          <span className="text-sm font-bold text-ink">{viewYear}年</span>
          <button
            type="button"
            onClick={() => setViewYear((y) => y + 1)}
            className="p-1 rounded-md text-ink-muted hover:bg-slate-100"
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isSelected ? 'bg-brand text-white' : 'text-ink-soft hover:bg-slate-100'
                )}
              >
                {label}
              </button>
        ...
        <button
          type="button"
          onClick={handleToday}
          className="w-full text-center text-xs font-bold text-brand hover:text-brand-strong py-1.5 rounded-md hover:bg-brand-soft transition-colors"
        >
          今月に戻る
        </button>
      </PopoverContent>
