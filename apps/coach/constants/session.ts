...
export const SESSION_STATUS_BADGE: Record<SessionStatus, { label: string; className: string; dotClassName: string }> = {
  ...
  [SESSION_STATUS.CANCELLED_LICENSE_ENDED]: { label: 'Cancelled (license ended)', className: 'bg-slate-100 text-slate-600 border-slate-200', dotClassName: 'bg-slate-400' },
  [SESSION_STATUS.CANCELLED_COACH_REASSIGNED]: { label: 'Cancelled (coach reassigned)', className: 'bg-slate-100 text-slate-600 border-slate-200', dotClassName: 'bg-slate-400' },
  [SESSION_STATUS.CANCELLED_BY_ADMIN]: { label: 'Cancelled', className: 'bg-slate-100 text-slate-600 border-slate-200', dotClassName: 'bg-slate-400' },
};
