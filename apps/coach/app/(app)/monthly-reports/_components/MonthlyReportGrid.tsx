...
function cellClassFor(sessions: MonthlyReportSession[]): string {
  ...
  if (sessions.some((s) => s.is_unresolved)) {
    return 'bg-amber-100 text-amber-800 font-bold cursor-pointer hover:bg-amber-200';
  }
...
export function MonthlyReportGrid({ report }: { report: CoachMonthlyReport }) {
  ...
  const openCell = (student: MonthlyReportStudentRow, dateKey: string, sessions: MonthlyReportSession[]) => {
    if (sessions.length === 0 || !sessions.some((s) => s.is_unresolved || s.is_attention)) return;
    setDetail({ studentId: student.student_id, userName: student.user_name, iconPath: student.icon_path, dateKey, sessions });
  };

  if (report.students.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-slate-200 py-12 text-center text-sm text-slate-400">
        No students with an active contract this month.
      </div>
    );
  }
  ...
  return (
    <>
              ...
              <tr key={student.student_id}>
                ...
                {days.map((day) => {
                  ...
                  return (
                    <td
                      key={day}
                      onClick={() => openCell(student, dateKey, sessions)}
                      className={`text-center align-middle py-2 ${cellClassFor(sessions)}`}
                    >
      ...
      <AttentionCellDialog detail={detail} onClose={() => setDetail(null)} />
    </>
  );
}
