 ...
 *   - booking: 未消化チケットによる自由日時の新規予約リクエスト
 *     (com_t_session_slot_proposal, source_session_id IS NULL)
 *   - reschedule_proposal: 生徒がキャンセル時に提案した振替候補
 *     (com_t_session_slot_proposal, source_session_id IS NOT NULL かつ proposed_by_role=1のみ。
 *     コーチ発の候補提案は生徒側で応答するため、この一覧には含めない)。
 *     pending・応答済み(承諾/却下)の両方を含む
 */
